...
export function formatCrowdSignalEvidence(crowd: CrowdSignalInput): string {
  ...
  if (typeof stocktwits?.bull_ratio === 'number') {
    const source = safeSourceLink(stocktwits.source || 'StockTwits', stocktwits.source_url);
    const labelled = stocktwits.total_with_sentiment ?? 0;
    const sampled = stocktwits.total_messages;
    const sample = sampled != null
      ? `${labelled} labelled messages out of ${sampled} sampled`
      : `${labelled} labelled messages`;
    const split = stocktwits.bullish != null && stocktwits.bearish != null
      ? ` (${stocktwits.bullish} bullish / ${stocktwits.bearish} bearish)`
      : '';
    observations.push(`${source} showed ${stocktwits.bull_ratio.toFixed(1)}% bullish across ${sample}${split}`);
  }
